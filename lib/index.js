...
const createDirectory = directoryName => {
  try {
    fs.mkdirSync(`${__dirname}/${directoryName}`);
  } catch (error) {
    console.error(error);
  }
};
...
const createIndexFile = (path, className) => {
  try {
    let file = fs.readFileSync(`${process.cwd()}/templates/index.tpl`, 'utf8');
...
const createPackageFile = (path, tagName) => {
  try {
    let file = fs.readFileSync(`${process.cwd()}/templates/package.tpl`, 'utf8');
...
const createElementFile = (path, tagName, className) => {
  try {
    let file = fs.readFileSync(`${process.cwd()}/templates/element.tpl`, 'utf8');
    file = file.replace(/{{TAG_NAME}}/gm, tagName);
...
const createBaseFiles = path => {
  ...
  for (const file of files) {
    try {
      const fileData = fs.readFileSync(`${process.cwd()}/templates/${file.tpl}.tpl`, 'utf8');
...
const createDemo = (path, tagName, className) => {
  try {
    ...
    let file = fs.readFileSync(`${process.cwd()}/templates/demo.tpl`, 'utf8');
    file = file.replace(/{{TAG_NAME}}/gm, tagName);
    file = file.replace(/{{CLASS_NAME}}/gm, className);
...
const createTest = (path, tagName) => {
  try {
    ...
    let file = fs.readFileSync(`${process.cwd()}/templates/test.tpl`, 'utf8');
    file = file.replace(/{{TAG_NAME}}/gm, tagName);
